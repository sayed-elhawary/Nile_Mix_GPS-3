...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      await logActivity(req, 'ADD_PENDING_VIOLATION', newViolation._id, {
        carCode,
        amount,
        type,
        hasImage: !!image
      ...
      res.status(201).json({
        message: 'تم إضافة المخالفة بنجاح (معلقة لإكمال HR)',
...
router.patch('/:id/complete', authMiddleware, allowRoles('hr'), async (req, res) => {
  try {
    ...
    await logActivity(req, 'COMPLETE_VIOLATION', req.params.id, {
...
// 3. جلب المخالفات المعلقة ← هنا التعديل المهم: خلينا الأدمن يشوفها كمان
...
// 4. جلب المخالفات المكتملة (للجميع)
